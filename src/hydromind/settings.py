...
class Settings(BaseModel):
    ...
    def from_env(cls) -> "Settings":
        load_dotenv()
        return cls(
            model=os.getenv("HYDROMIND_MODEL", "test"),
            model_provider=os.getenv("HYDROMIND_MODEL_PROVIDER", "auto"),
            ...
            http_timeout_seconds=float(
                os.getenv("HYDROMIND_HTTP_TIMEOUT_SECONDS", "30")
            ),
            metoffice_sample_grid_size=int(
                os.getenv("HYDROMIND_METOFFICE_SAMPLE_GRID_SIZE", "5")
            ),
            user_agent=os.getenv("HYDROMIND_USER_AGENT", "hydromind/0.1"),
            default_area=os.getenv("HYDROMIND_DEFAULT_AREA", "glasgow"),
            geoserver_wms_url=os.getenv(
                "HYDROMIND_GEOSERVER_WMS_URL",
                "http://127.0.0.1:8080/geoserver/glasgow_flood/wms",
            ),
            geoserver_rest_url=os.getenv(
                "HYDROMIND_GEOSERVER_REST_URL", "http://127.0.0.1:8080/geoserver/rest"
            ),
            geoserver_user=os.getenv("HYDROMIND_GEOSERVER_USER", "admin"),
            geoserver_password=SecretStr(
                os.getenv("HYDROMIND_GEOSERVER_PASSWORD", "geoserver")
            ),
            current_hazard_layer=os.getenv(
                "HYDROMIND_CURRENT_HAZARD_LAYER",
                "glasgow_flood:current_hazard_class_5m",
            ),
            current_hazard_raster_path=Path(
                os.getenv(
                    "HYDROMIND_CURRENT_HAZARD_RASTER_PATH",
                    "webgis/.runtime/data_dir/data/glasgow_flood/"
                    "current_hazard_class_5m/current_hazard_class_5m.geotiff",
                )
            ),
            current_hazard_output_dir=Path(
                os.getenv(
                    "HYDROMIND_CURRENT_HAZARD_OUTPUT_DIR",
            ...
            core_analyst_config_path=Path(
                os.getenv(
                    "HYDROMIND_CORE_ANALYST_CONFIG_PATH",
                    "analysis/core-analyst/config/pluvial_prediction_config.yaml",
                )
            ),
            core_analyst_config_dir=Path(
                os.getenv(
                    "HYDROMIND_CORE_ANALYST_CONFIG_DIR",
                    "analysis/core-analyst/config",
                )
            ),
            core_analyst_analysis_output_dir=Path(
                os.getenv(
                    "HYDROMIND_CORE_ANALYST_ANALYSIS_OUTPUT_DIR",
                    "analysis/core-analyst/outputs/agent",
                )
            ),
            lcm2019_path=(
                Path(value) if (value := os.getenv("HYDROMIND_LCM2019_PATH")) else None
            ),
            accept_data_licences=_env_bool("HYDROMIND_ACCEPT_DATA_LICENCES", False),
            auto_prepare_data=_env_bool("HYDROMIND_AUTO_PREPARE_DATA", True),
            ceda_access_token=(
            ...
            historical_ukv_path=(
                Path(value) if (value := os.getenv("HYDROMIND_HISTORICAL_UKV_PATH")) else None
            ),
            nominatim_url=os.getenv(
                "HYDROMIND_NOMINATIM_URL",
