...
def _model_configuration(settings: Settings) -> tuple[list[str], str]:
    ...
    if settings.model_provider == "vllm":
        return ["HYDROMIND_MODEL", "HYDROMIND_MODEL_PROVIDER", "OPENAI_BASE_URL", "OPENAI_API_KEY"], (
    ...
    if settings.model_provider == "mimo":
        return ["HYDROMIND_MODEL", "HYDROMIND_MODEL_PROVIDER", "MIMO_API_KEY"], "MiMo requires MIMO_API_KEY."
    return ["HYDROMIND_MODEL", "OPENAI_API_KEY"], "Hosted OpenAI mode requires a live model identifier and OPENAI_API_KEY."
